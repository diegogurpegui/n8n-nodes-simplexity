...
- **Audio flow:** Voice message → `newChatItems` with voice content and `chatItem.file.fileStatus.type === 'rcvInvitation'` → call ReceiveFile with `fileInline: true` (voice uses inline transfer) → `rcvFileComplete` (file ready with filePath). Standalone files use `apiReceiveFile(fileId)` without fileInline. Do not call `apiReceiveFile` on `rcvFileAccepted`; it causes "error receiving file".
